...
- 目标（用户价值）：
  ...
  - Prompt 透明度与可编辑性（关键交互原则）：
    ...
    - 提示词“组合与编排规则”本身需要可观察与可配置（建议提供设置页 UI，便于随时查看/调整）
  ...
  - 需要在契约文档中正式定义（或补齐）以下“长期稳定”的对象：
    ...
    - 统一的“生成版本/产物”模型（建议所有生成类动作都落到同一套抽象）：
      ...
      - GenerationArtifact：生成产物（文本/图片/JSON/文件），以“文件路径 + 元信息”方式存储，避免 base64 直接入库
        - 图片产物建议附带：
          - effectivePrompt：含上下文拼接后的最终 prompt（UI 必须可见、可复制；默认编辑的是 draft，避免“把 effectivePrompt 再次拼接”导致套娃）
...
- [ ] 通用图片生成/编辑组件（Lite）可用：在任意“需要出图”的页面复用；展示服务端回显的 effectivePrompt 并支持复制；默认编辑的是 draft prompt（服务端会按规则拼接上下文生成 effectivePrompt）；支持重新生成/重新编辑；每次动作创建新版本且原图保留。
