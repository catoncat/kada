...
**拍摄预案助手 v2**：输入主题（或 1–3 套服装与客户信息），自动生成“分镜结构 + 镜头建议 + 生图提示词”，并可按需生成参考图与导出 PPT，用于拍摄前沟通与执行。
...
- **单预案（Single Plan）**：一个主题 → 多个场景（location/description/shots/visualPrompt）。
- **三套服装项目（Project Plan）**：每套服装固定输出 4 个场景（主场景、叠搭A、叠搭B、纯色版面），可嵌入统一的“全局风格锚点”。
- **参考图（可选）**：根据每个场景的 `visualPrompt` 调用图片生成模型得到 Base64 图片。
...
- 生成方式：
  - 普通主题：输入/选择主题 → 生成单预案
  - 3 套服装：输入客户信息 + 1–3 套服装 + 全局风格 → 生成项目预案
- 参考图：
...
- 历史记录：最近 20 条预案保存在本地（当前实现为浏览器 localStorage）
