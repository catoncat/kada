...
test('action endpoints return same SESSION_NOT_RUNNING error for both engines', async () => {
  await ensureDb();
  ...
  try {
    for (const scenario of scenarios) {
      const codingRes = await postJson(scenario.path(coding.id), scenario.body);
...
test('turn invalid payload returns same INVALID_PAYLOAD error for both engines', async () => {
  await ensureDb();
  const coding = await createSession('coding-agent');
  const core = await createSession('agent-core');
...
test('archived session rejection is consistent for both engines', async () => {
  await ensureDb();
  const coding = await createSession('coding-agent');
  const core = await createSession('agent-core');
