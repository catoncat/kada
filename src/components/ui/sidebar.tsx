...
const handleSidebarDrag = async (e: React.MouseEvent) => {
  ...
  if (
    target.closest(
      'button, a, input, [role="button"], [data-slot="scroll-area-thumb"]',
    )
  )
    return;
  try {
...
function SidebarProvider({
  ...
  return (
    <SidebarContext.Provider value={contextValue}>
      <div
        className={cn(
          // 固定应用高度，让内部滚动容器可计算高度（避免 body 滚动/双滚动）
...
function Sidebar({
  ...
  if (collapsible === 'none') {
    return (
      <div
        className={cn(
          'app-sidebar-surface flex h-full w-(--sidebar-width) flex-col text-sidebar-foreground',
          className,
        )}
        data-slot="sidebar"
        {...props}
  ...
  return (
    ...
    >
      ...
      >
        <div
          className="app-sidebar-surface flex h-full w-full select-none flex-col group-data-[variant=floating]:rounded-lg group-data-[variant=floating]:border group-data-[variant=floating]:border-sidebar-border group-data-[variant=floating]:shadow-sm/5"
          data-sidebar="sidebar"
...
function SidebarInput({
  ...
  return (
    <Input
      className={cn(
        'h-[var(--control-h)] w-full bg-background shadow-none',
        className,
      )}
      data-sidebar="input"
...
function SidebarContent({ className, ...props }: React.ComponentProps<'div'>) {
  return (
    ...
    >
      <div
        className={cn(
          'flex min-h-0 flex-1 flex-col gap-1.5 overflow-x-hidden overflow-y-auto group-data-[collapsible=icon]:overflow-hidden',
          className,
        )}
...
function SidebarGroupLabel({
  ...
  const defaultProps = {
    className: cn(
      'flex h-6 shrink-0 items-center px-2 font-medium text-[10px] text-sidebar-foreground/62 tracking-[0.02em] uppercase outline-hidden ring-sidebar-ring transition-[margin,opacity] duration-200 ease-linear focus-visible:ring-2 [&>svg]:size-3.5 [&>svg]:shrink-0',
...
const sidebarMenuButtonVariants = cva(
  "peer/menu-button flex w-full items-center gap-2 overflow-hidden rounded-md px-2 text-left text-sm outline-hidden ring-sidebar-ring/50 transition-[width,height,padding,background-color,color] hover:bg-sidebar-accent hover:text-sidebar-accent-foreground focus-visible:ring-2 active:bg-sidebar-accent disabled:pointer-events-none disabled:opacity-50 group-has-data-[sidebar=menu-action]/menu-item:pe-8 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-[active=true]:bg-[var(--sidebar-selection)] data-[active=true]:font-medium data-[active=true]:text-[var(--sidebar-selection-foreground)] data-[state=open]:hover:bg-sidebar-accent group-data-[collapsible=icon]:size-8! group-data-[collapsible=icon]:p-2! [&>span:last-child]:truncate [&>svg:not([class*='size-'])]:size-4 [&>svg]:shrink-0",
  {
    defaultVariants: {
      size: 'default',
      variant: 'default',
    ...
    variants: {
      size: {
        default: 'h-[var(--control-h)] text-sm',
