    ...
    >
      ...
      <div className="relative aspect-[4/3] bg-[var(--paper-2)]">
        {scene.primaryImage ? (
          ...
          <div className="absolute inset-0 flex items-center justify-center">
            <ImageIcon className="w-12 h-12 text-[var(--ink-3)] opacity-50" />
          </div>
        )}
      ...
      <div className="p-4">
        ...
        {scene.tags && scene.tags.length > 0 && (
          <div className="mt-3 flex flex-wrap gap-1.5">
            {scene.tags.slice(0, 3).map((tag) => (
              <span
                key={tag}
                className="px-2 py-0.5 rounded-full bg-[var(--paper-2)] text-xs text-[var(--ink-2)]"
              >
                {tag}
              </span>
            ))}
            {scene.tags.length > 3 && (
              <span className="px-2 py-0.5 text-xs text-[var(--ink-3)]">
                +{scene.tags.length - 3}
              </span>
            )}
          </div>
        ...
        {scene.defaultLighting && (
          <div className="mt-3 flex items-center gap-1.5 text-xs text-[var(--ink-3)]">
        ...
        {!selectable && (onEdit || onDelete) && (
          <div className="mt-4 flex items-center gap-2 pt-3 border-t border-[var(--line)]">
            {onEdit && (
              <button
                type="button"
                onClick={(e) => {
                ...
                className="flex-1 inline-flex items-center justify-center gap-1.5 py-2 rounded-lg text-sm font-medium text-[var(--ink)] hover:bg-[var(--paper-2)] transition"
              >
            ...
            {onDelete && (
              <button
                ...
                className="flex-1 inline-flex items-center justify-center gap-1.5 py-2 rounded-lg text-sm font-medium text-red-500 hover:bg-red-50 transition"
              >
